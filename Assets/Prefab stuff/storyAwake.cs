...
public class story : MonoBehaviour, INode
{
    ...
    private void LoadStoryData()
    {
        if (File.Exists(savePath))
        {
            ...
            StoryDataList storyDataList = JsonUtility.FromJson<StoryDataList>(json);

            StoryData story = storyDataList.stories.Find(s => s.storyID == _storyID);
            if (story != null)
            {
